...
window.onload = function() {
  ...
  // Check if the GIF has been played before
  var hasPlayed = localStorage.getItem('hasPlayed');
  
  if (!hasPlayed) {
    // Play the GIF once
    gif.src = gif.src; // Restart the GIF by resetting its src attribute
    localStorage.setItem('hasPlayed', true); // Set hasPlayed to true in localStorage
  }
};
...
function switcher(panelIndex) {
  ...
  allButtons[panelIndex].style.background = " radial-gradient(circle, #869191, #789191, #689191, #569091, #419091)";
